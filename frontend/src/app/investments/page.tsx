...
export default function InvestmentsDashboardPage() {
  ...
  const [holdingsReturnMode, setHoldingsReturnMode] = useState<"total" | "1d">("total");
            ...
            <div className="flex items-center justify-between pb-3 mb-2 border-b border-[#F1F5F9] dark:border-slate-800">
              <div className="flex items-center gap-3">
                ...
                <div className="flex items-center gap-1 bg-slate-100 dark:bg-slate-800 p-0.5 rounded-lg text-xs font-semibold">
                  ...
                  <button
                    type="button"
                    onClick={() => setHoldingsReturnMode("1d")}
                    className={`px-2 py-0.5 rounded-md text-[11px] font-bold transition-all cursor-pointer ${
                      holdingsReturnMode === "1d"
                        ? "bg-white dark:bg-slate-700 text-[#0F172A] dark:text-white shadow-xs"
                        : "text-slate-500 hover:text-slate-900 dark:text-slate-400"
                    }`}
                  >
                ...
                <thead>
                  <tr className="text-[11px] font-bold text-slate-400 border-b border-slate-100 dark:border-slate-800">
                    ...
                    <th className="py-2.5 px-3 text-right font-semibold">
                      {holdingsReturnMode === "1d" ? "1D Return ↓" : "P/L ↓"}
                    </th>
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800 tabular-nums">
                  {summary?.top_holdings && summary.top_holdings.length > 0 ? (
                    summary.top_holdings.map((h) => {
                      ...
                      return (
                        <tr key={h.asset_id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/40 transition-colors group">
                          {/* Title Column */}
                          <td className="py-3 px-2">
                            <div className="flex items-center gap-3">
                              {/* 3-letter Ticker Badge Avatar */}
                              <div className="w-8 h-8 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-800 dark:text-slate-100 font-extrabold text-[10px] flex items-center justify-center border border-slate-200/80 dark:border-slate-700 shrink-0">
                                {initials}
                              </div>
                              <div className="truncate max-w-[200px] sm:max-w-xs">
                                <div className="font-bold text-[#0F172A] dark:text-white text-xs truncate">
                                  {h.name || h.symbol}
                                </div>
                                <div className="text-[11px] font-medium text-slate-400 flex items-center gap-1.5 mt-0.5">
                                  <span className="font-semibold text-slate-600 dark:text-slate-300">{h.symbol}</span>
                                  <span className="bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 text-[10px] font-bold px-1.5 py-0.2 rounded">
                          ...
                          {/* P/L or 1D Return Column (Native Currency) */}
                          <td className="py-3 px-3 text-right">
                            {holdingsReturnMode === "1d" ? (
                              (() => {
                                ...
                                return (
                                  <>
                                    <div className={`font-bold text-xs ${isPos1d ? "text-[#16A34A] dark:text-emerald-400" : "text-[#DC2626] dark:text-rose-400"}`}>
                                      {hideBalances ? "••••" : formatMoney(val1d, curr, 2, true)}
                                    </div>
                              ...
                              <>
                                <div className={`font-bold text-xs ${isPositive ? "text-[#16A34A] dark:text-emerald-400" : "text-[#DC2626] dark:text-rose-400"}`}>
                                  {hideBalances ? "••••" : formatMoney(h.unrealized_pnl, curr, 2, true)}
                                </div>
                                <div className={`text-[11px] font-semibold flex items-center justify-end gap-0.5 mt-0.5 ${isPositive ? "text-[#16A34A] dark:text-emerald-400" : "text-[#DC2626] dark:text-rose-400"}`}>
                          ...
                          <td className="py-3 px-2 text-right">
                            <button className="p-1 text-slate-300 hover:text-slate-700 dark:hover:text-slate-200 rounded transition-colors">
